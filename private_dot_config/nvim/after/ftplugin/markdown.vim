...
nnoremap <silent><buffer> <space>ac :call CreateNotesCloze()<cr>
...
function! CreateNotesCloze() abort " {{{1
  " Create notes from list of question/answers
  "
  " <category>
  " tags: list (default = category)
  ...
  for l:line in l:lines
    ...
    if l:line =~# '^T:'
      ...
      let l:current.tags = l:tags
      let l:current.t = [matchstr(l:line, '^T: \zs.*')]
      let l:current.pointer = l:current.t
    elseif l:line =~# '^E:'
      let l:current.e = [matchstr(l:line, '^E: \zs.*')]
      let l:current.pointer = l:current.e
    else
      let l:current.pointer += [l:line]
    endif
  endfor
  ...
  " Add new notes
  for l:e in l:list
    let l:t = escape(join(l:e.t, "\n"), '\')
    let l:e = escape(join(l:e.e, "\n"), '\')

    let l:new = copy(l:template)
    let l:new = substitute(l:new, '{tags}', l:e.tags, 'g')
    let l:new = substitute(l:new, '{t}', l:t, 'g')
    let l:new = substitute(l:new, '{e}', l:e, 'g')
    let l:new = substitute(l:new, "  \n", "\n\n", 'g')
    call append(line('.')-1, split(l:new, "\n") + [''])
  endfor
  silent execute line('.') . 'd'
